...
计划版本：`v1.0`。计划状态：`READY`（2026-08-17 用户裁决：四项设计决策按建议值通过）。交付状态：`DESIGNING`。
...
1. **转账即时性**：即时到账（无确认等待——账本内操作，非链上）。
2. **领取链接过期**：默认 24 小时，可配置；过期自动退款（惰性检查——领取时发现过期则退款）。
3. **红包类型**：先做普通固定金额红包，拼手气留后续版本。
4. **费用**：内部转账免费；归集/提现/换汇才有链上或供应商费用。
