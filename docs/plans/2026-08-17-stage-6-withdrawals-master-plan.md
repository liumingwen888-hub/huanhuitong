...
计划版本：`v1.0`。计划状态：`READY`（2026-08-17 用户裁决：五项设计决策按建议值通过）。交付状态：`DESIGNING`。

基线：阶段 1–5 VERIFIED（2026-08-17 用户验收通过）。

阶段 6 状态：**代码 READY，等待用户验收**（2026-08-19：S6-1～S6-8 全部实施 VERIFIED；S6A01-12 验收 12/12 PASS；全量回归绿——unit 235、database 440/443（M06/M14/M16 已知环境边界）、integration 97/97）。
...
## 已裁决的设计决策（2026-08-17 用户通过）

1. **提现审批模式**：低额自动 / 高额人工双轨。
2. **Signer 隔离级别**：同进程接口隔离；独立容器留后续。
3. **热/冷钱包策略**：初期全热钱包；金额阈值分离留后续。
4. **提现费用**：固定费。
5. **每日限额**：复用 S3-4 operation_limits。
