...
## 阶段 10 威胁模型增补（2026-08-19，S10-8）

| 威胁 | 控制 | 证据 |
|---|---|---|
| 备份文件泄露（含全量用户与账本数据） | 备份脚本零明文凭据（PG* 环境变量）+ sha256 校验和 + manifest 审计；异地存储属生产独立授权 | S10BK01 |
| 恢复演练重发副作用（恢复被当作重发器） | Outbox 未投递原样保留断言 + durable_jobs 零新 LEASED 断言 + 演练脚本硬编码检查 | S10DR04 |
| 发布绕过（未过门禁直接部署） | release:gate 八门禁 fail-fast + 无 --force 逃生口 + BLOCKED 清单非零退出 | S10RG02 |
| 观测盲区（关键路径无指标） | 12 计数器封闭枚举 + 六域服务层插桩（域内零渗透）+ 告警规则声明式白名单 | S10OB04 |
